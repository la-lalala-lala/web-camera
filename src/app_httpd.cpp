...
// 串口指令转发
static esp_err_t stm32_handler(httpd_req_t *req)
{
    char *buf = NULL;
    ...
    // 功能码
    int fun = parse_get_var(buf, "fun", 0);
    // 指令值
    int val = parse_get_var(buf, "val", 0);
    free(buf);
    log_i("Get Command: fun: %d, val: %d", fun, val);
    static char json_response[1024];
    char *p = json_response;
    *p++ = '{';
    p += sprintf(p, "\"fun\":%d,", fun);
    p += sprintf(p, "\"val\":%d,", val);
    *p++ = '}';
    *p++ = 0;
    //Serial1.print("111");
    httpd_resp_set_type(req, "application/json");
...
// 回调函数处理WebSocket连接
// static esp_err_t ws_handler(esp_http_client_handle_t client, void *user_data) {
//     // 检查WebSocket握手
//     if (esp_http_client_is_websocket_handshake(client)) {
//         ESP_LOGI(TAG, "WebSocket handshake received");
//         // 接受握手
//         esp_http_client_accept_websocket_handshake(client, NULL);
//     } else {
//         // 处理WebSocket数据帧
//         int data_len;
//         char* buffer = (char*)malloc(512);
//         while ((data_len = esp_http_client_websocket_read(client, buffer, 512, 1000 / portTICK_RATE_MS)) > 0) {
//             // 对接收到的数据进行处理
//             // ...
 
//             // 发送数据回客户端
//             // esp_http_client_websocket_write(client, data, length, WS_OP_TEXT);
//         }
//         free(buffer);
//     }
//     return ESP_OK;
...
void startCameraServer()
{
    ...
    httpd_uri_t stm32_uri = {
        .uri = "/stm32",
        .method = HTTP_GET,
        .handler = stm32_handler,
        .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
        ,
        .is_websocket = true,
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(camera_httpd, &stm32_uri);
        //httpd_register_uri_handler(camera_httpd, "/ws", HTTPD_METHOD_GET, ws_handler, NULL);
